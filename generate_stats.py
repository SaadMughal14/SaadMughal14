...
def gen_langs_svg(lang_sizes, path, top_n=5):
    ...
    for i, (name, size) in enumerate(top):
        ...
        rows.append(f'''<text class="label" x="0" y="{y-40}">{name}</text><text class="pct" x="260" y="{y-40}">{pct:.0f}%</text>
  ...
  <rect x="0" y="{y-34}" width="0" height="8" rx="4" fill="{color}">
    <animate attributeName="width" from="0" to="{bar_w}" begin="{0.3+i*0.2:.1f}s" dur="1s" fill="freeze" calcMode="spline" keySplines="0.2 0 0.2 1"/>
        ...
        y += 34
    svg = f'''<svg width="320" height="195" viewBox="0 0 320 195" xmlns="http://www.w3.org/2000/svg">
<defs><style>.sans{{font-family:'Segoe UI',Helvetica,Arial,sans-serif;}}.label{{fill:#e3edff;font-size:13px;}}.pct{{fill:#7a8fa8;font-size:12px;}}</style></defs>
<rect x="0.5" y="0.5" width="319" height="194" rx="16" fill="{BG}" stroke="rgba(59,130,246,0.25)"/>
